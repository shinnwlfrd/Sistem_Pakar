...
penyakit = {
    ...
    'P4': 'Penyakit Karat Daun',
    'P5': 'Penyakit Jamur Upas'
}
...
likelihood = {
    'P1': {'G1': 0.8, 'G2': 0.9, 'G3': 0.7, 'G4': 0.1, 'G5': 0.4, 'G6': 0.5, 'G7': 0.6, 'G8': 0.1, 'G9': 0.3},
    'P2': {'G1': 0.4, 'G2': 0.1, 'G3': 0.1, 'G4': 0.9, 'G5': 0.3, 'G6': 0.2, 'G7': 0.3, 'G8': 0.8, 'G9': 0.2},
    'P3': {'G1': 0.5, 'G2': 0.2, 'G3': 0.5, 'G4': 0.2, 'G5': 0.9, 'G6': 0.7, 'G7': 0.7, 'G8': 0.1, 'G9': 0.6},
    'P4': {'G1': 0.3, 'G2': 0.1, 'G3': 0.2, 'G4': 0.8, 'G5': 0.2, 'G6': 0.6, 'G7': 0.4, 'G8': 0.7, 'G9': 0.1},
    'P5': {'G1': 0.4, 'G2': 0.3, 'G3': 0.5, 'G4': 0.3, 'G5': 0.6, 'G6': 0.8, 'G7': 0.6, 'G8': 0.2, 'G9': 0.7}
}
...
def diagnosa(gejala_teramati):
    """
    Menghitung probabilitas posterior untuk setiap penyakit
    berdasarkan gejala yang teramati menggunakan Teorema Bayes.
    """
    posterior_unnormalized = {}
    
    for p_kode in penyakit.keys():
        prob = prior[p_kode]
    ...
    if total_prob > 0:
        posterior_normalized = {
            p_kode: prob / total_prob 
            for p_kode, prob in posterior_unnormalized.items()
        }
    else:
        posterior_normalized = prior.copy()
...
if st.button("🔍 Jalankan Diagnosa"):
    if len(selected_gejala) < 3:
        ...
        for p_kode, prob in sorted(hasil_probabilitas.items(), key=lambda x: x[1], reverse=True):
    ...
    st.info("Pilih minimal 3 gejala, lalu tekan tombol *Jalankan Diagnosa* untuk melihat hasil.")def diagnosa(gejala_teramati):
    """
